...
<body class="bg-slate-950 h-screen overflow-hidden text-slate-300 selection:bg-blue-500/30">
    <div id="app" class="flex flex-col h-full w-full" v-cloak>
        <header class="bg-slate-950 border-b border-slate-900 h-16 shrink-0 flex items-center justify-between px-8 z-30 relative shadow-2xl shadow-black/50">
            <div class="flex items-center gap-4">
                <div class="bg-blue-600 p-1.5 rounded-none shadow-lg">
            ...
            <div class="flex items-center gap-6">
                <button v-if="currentUser" @click="showProfileModal = true" class="text-[10px] text-slate-500 hover:text-white font-black uppercase tracking-widest">{{ currentUser.nickname || currentUser.username }}</button>
        ...
        <div class="flex-1 flex overflow-hidden w-full relative">
            <aside class="bg-slate-950 flex flex-col shrink-0 z-10 relative border-r border-slate-900" :style="{ width: leftColWidth + 'px' }">
                <div class="flex flex-col flex-1 overflow-hidden">
                    <div class="h-1/2 min-h-[200px] border-b border-slate-900 flex flex-col bg-slate-950 shrink-0">
                        <div class="p-6 flex justify-between items-center"><span class="font-black text-[10px] uppercase tracking-[0.2em] text-slate-700">Channels</span><button @click="createRoom" class="text-slate-700 hover:text-white">+</button></div>
                        <div class="flex-1 overflow-y-auto px-3 space-y-1">
                            <div v-for="room in rooms" :key="room.id" @click="selectRoom(room)" class="nav-item px-4 py-2.5 rounded-none cursor-pointer text-sm flex justify-between items-center group transition-all" :class="activeRoom?.id === room.id ? 'bg-slate-900 text-white font-bold nav-item-active' : 'text-slate-600 hover:bg-slate-900/50 hover:text-slate-400'">
                                ...
                                <button @click.stop="openSettings(room)" class="opacity-0 group-hover:opacity-100 text-slate-700 hover:text-white">⚙️</button>
                            </div>
                        </div>
                    </div>
                    <div class="flex-1 flex flex-col overflow-hidden bg-slate-950">
                        <div class="p-4 bg-slate-950 border-b border-slate-900 flex justify-between items-center"><span class="font-black text-[10px] uppercase text-slate-700 tracking-widest ml-2">Stream</span><button v-if="activeRoom && roomMessages.length > 0" @click="confirmClearMessages" class="text-[9px] uppercase font-bold text-slate-800 hover:text-red-500">Wipe</button></div>
                        <div class="flex-1 overflow-y-auto p-6 space-y-6" id="webhook-stream">
                            <div v-for="msg in roomMessages" :key="msg.id" class="group">
                                ...
                                <div class="font-mono text-slate-400 text-xs leading-relaxed break-words bg-slate-900/30 p-3 rounded-none border-none" v-html="linkify(msg.content)"></div>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <ul class="space-y-1">
                                <template v-for="page in getPages(space.id)" :key="page.id">
                                    <li @click="selectDoc(page)" class="nav-item group flex flex-col cursor-pointer">
                                        <div class="flex items-center justify-between pl-3 pr-2 py-2 rounded-none transition-all" :class="activePage?.id == page.id ? 'bg-slate-950 text-white font-bold nav-item-active' : 'text-slate-500 hover:bg-slate-950/30 hover:text-slate-300'">
                                            <div class="nav-indicator"></div>
                                            <span class="truncate text-sm">{{ page.has_draft ? page.draft_title : page.title }}</span>
                                            <div class="flex items-center gap-2">
                                                <span v-if="page.is_public == 1" class="text-[8px] bg-green-500/10 text-green-500 px-1 rounded-none font-black uppercase">Live</span>
                                                <button @click.stop="createItem('subpage', page.id)" class="hidden group-hover:block text-slate-600 hover:text-blue-400 text-xl">+</button>
                                            </div>
                                        </div>
                                        <ul class="mt-1 ml-4 border-l border-slate-950">
                                            <li v-for="subpage in getSubpages(page.id)" :key="subpage.id" @click.stop="selectDoc(subpage)" class="nav-item pl-4 pr-2 py-1.5 rounded-none text-xs transition-all flex items-center justify-between" :class="activePage?.id == subpage.id ? 'bg-slate-950 text-blue-400 font-bold nav-item-active' : 'text-slate-600 hover:bg-slate-950/20 hover:text-slate-400'">
                                                <div class="nav-indicator"></div>
            ...
            <main class="flex-1 flex flex-col bg-slate-950 relative overflow-hidden">
                ...
                <template v-if="activePage">
                    <header class="relative border-b border-slate-900 flex flex-col justify-end shrink-0 transition-all duration-700 ease-in-out" :class="hasCover(activePage) ? 'h-80' : 'h-36 bg-slate-950'" :style="getCoverStyle(activePage)">
                        ...
                        <div class="absolute top-8 right-8 z-20 flex gap-3">
                            ...
                            <button v-if="hasCover(activePage)" @click="confirmRemoveBanner" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-none text-[10px] font-black uppercase shadow-xl transition-all">Remove</button>
                            <button @click="triggerCoverUpload" class="bg-white hover:bg-slate-200 text-black px-4 py-2 rounded-none text-[10px] font-black uppercase shadow-2xl transition-all">
                        ...
                        <div class="relative z-10 p-12 pb-8 flex justify-between items-end w-full mt-auto max-w-5xl mx-auto pl-20 md:pl-12">
                            <div class="w-full">
                                <input v-model="activePage.title" placeholder="Untitled Page" class="text-6xl font-black bg-transparent text-white outline-none w-full drop-shadow-2xl focus:placeholder-slate-800 transition-all">
                                <div v-if="activePage.is_public == 1" class="mt-4 animate-in fade-in slide-in-from-left-4 duration-500">
                                    <a :href="'p.php?s=' + activePage.slug" target="_blank" class="inline-flex items-center gap-2 bg-blue-600/20 hover:bg-blue-600/30 text-blue-500 px-4 py-2 text-[10px] font-black uppercase tracking-widest transition-all">
                            ...
                            <div class="flex items-center space-x-6 shrink-0 mb-3 ml-8">
                                ...
                                <button @click="confirmDelete(activePage.id, 'page')" class="text-slate-700 hover:text-red-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                </button>
                            </div>
                        </div>
                    </header>
                    <div class="bg-slate-950 border-b border-slate-900/50 p-3 px-12 flex justify-between items-center shrink-0 text-[10px] font-black uppercase text-slate-700 tracking-widest">
                        <span v-if="lastSaveTime" class="text-blue-700">Auto-saved Signal: {{ lastSaveTime }}</span>
                        <span v-if="activePage.has_draft == 1" class="text-amber-600 animate-pulse">Modified Draft</span>
                    </div>
                ...
                <div v-else class="flex-1 flex flex-col items-center justify-center text-slate-900 opacity-20">
        ...
        <transition name="modal">
            <div v-if="alertDialog.show" class="fixed inset-0 z-[600] bg-black/80 flex items-center justify-center p-4">
                <div class="bg-slate-900 p-10 rounded-none w-full max-w-md shadow-2xl text-center border border-slate-800">
                    ...
                    <p class="text-slate-500 text-sm mb-10">{{ alertDialog.message }}</p>
                    <button @click="alertDialog.show = false" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-none text-xs shadow-xl transition-all">Continue</button>
                </div>
            </div>
        </transition>
            ...
            <div v-if="showUsersModal" class="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-6">
                <div class="bg-slate-950 p-12 rounded-none border border-slate-900 w-full max-w-6xl shadow-2xl flex flex-col max-h-[90vh]">
                    ...
                    <div class="flex flex-col md:flex-row gap-12 overflow-hidden">
                        <div class="w-full md:w-96 bg-slate-900 p-8 border border-slate-950 rounded-none shadow-xl">
                            ...
                            <form @submit.prevent="saveUser" class="space-y-5">
                                ...
                                <div class="flex gap-3 pt-6">
                                    <button type="submit" class="bg-blue-600 hover:bg-blue-500 text-white px-6 py-4 rounded-none text-xs font-black uppercase tracking-widest w-full transition-all shadow-xl shadow-blue-900/30">{{ editingUser ? 'Apply' : 'Invite' }}</button>
                                    <button v-if="editingUser" type="button" @click="cancelEditUser" class="bg-slate-800 hover:bg-slate-700 text-white px-6 py-4 rounded-none text-xs font-bold transition-all">Cancel</button>
                                </div>
                            </form>
                        </div>
                        <div class="flex-1 overflow-y-auto bg-slate-900/30 border border-slate-900 rounded-none">
                            <table class="w-full text-left text-sm border-separate border-spacing-0">
                                <thead class="sticky top-0 bg-slate-900 text-[10px] font-black uppercase text-slate-600 tracking-[0.2em]">
                                    <tr><th class="p-6 border-b border-slate-800">Identity</th><th class="p-6 border-b border-slate-800">Access</th><th class="p-6 border-b border-slate-800 text-right">Actions</th></tr>
                                </thead>
                                <tbody class="divide-y divide-slate-800/50">
                                    <tr v-for="user in userList" :key="user.id" class="hover:bg-blue-600/5 transition-colors">
                                        <td class="p-6"><span class="font-black text-white uppercase tracking-tight">{{ user.nickname || 'Pending' }}</span><span class="block text-[10px] text-slate-700 font-mono mt-1">{{ user.email }}</span></td>
        ...
        <transition name="modal">
            <div v-if="showProfileModal" class="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-6">
                <div class="bg-slate-900 p-12 rounded-none border border-slate-800 w-full max-w-md shadow-2xl">
                    <div class="flex justify-between items-center mb-10">
                    ...
                    <form @submit.prevent="updateProfile" class="space-y-6">
                        ...
                        <div class="pt-6"><button type="submit" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-none text-xs uppercase tracking-widest transition-all shadow-xl shadow-blue-900/30">Save Changes</button></div>
                    </form>
                </div>
            </div>
        </transition>

        <div v-if="showPromptModal" class="fixed inset-0 z-[150] bg-black/95 flex items-center justify-center p-6">
            <div class="bg-slate-900 p-12 border border-slate-800 w-full max-w-md shadow-2xl rounded-none">
                ...
                <form @submit.prevent="submitPrompt">
                    ...
                    <div class="flex gap-4">
                        ...
                        <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-4 rounded-none text-xs font-black uppercase tracking-widest shadow-xl transition-all">Confirm</button>
                    </div>
                </form>
            </div>
        </div>

        <div v-if="showSettingsModal" class="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-6">
            <div class="bg-slate-900 p-12 border border-slate-800 w-full max-w-lg shadow-2xl rounded-none text-center">
                <h2 class="text-2xl font-black text-white mb-8 uppercase tracking-tighter">Room Configuration</h2>
                <div class="bg-slate-950 p-8 mb-10 rounded-none border border-slate-800">
                    ...
                    <div v-if="settingsRoom.webhook_key" class="flex flex-col gap-4">
                        <input type="text" readonly :value="getWebhookUrl(settingsRoom.webhook_key)" class="w-full bg-slate-900 border border-slate-800 rounded-none p-4 text-[10px] text-green-500 font-mono text-center">
                        <button @click="confirmDeleteWebhook" class="text-red-700 text-[10px] font-black uppercase tracking-widest hover:text-red-500 transition-colors">Revoke Link</button>
                    </div>
                    <button v-else @click="generateWebhook" class="bg-blue-600 hover:bg-blue-500 text-white px-4 py-4 rounded-none text-xs font-black uppercase tracking-widest w-full transition-all">Enable Signalling</button>
                </div>
                <div class="flex justify-between items-center px-4">
                    <button @click="confirmDeleteRoom" class="text-red-900 hover:text-red-500 text-[10px] font-black uppercase tracking-widest transition-colors">Destroy Channel</button>
                    <button @click="showSettingsModal = false" class="bg-slate-800 hover:bg-slate-700 text-white px-10 py-3 rounded-none text-xs font-black uppercase tracking-widest transition-all">Close</button>
                </div>
            </div>
        </div>
        ...
        <div v-if="showCropModal" class="fixed inset-0 z-[200] bg-black/98 flex items-center justify-center p-4 backdrop-blur-3xl">
            <div class="bg-slate-950 border border-slate-900 w-full max-w-5xl flex flex-col shadow-2xl overflow-hidden rounded-none">
                <div class="p-10 border-b border-slate-900 flex justify-between items-center bg-slate-950">
                    ...
                    <button @click="cancelCrop" class="text-slate-600 hover:text-white">✕</button>
                </div>
                ...
                <div class="p-10 border-t border-slate-900 flex justify-end gap-6 bg-slate-950">
                    ...
                    <button @click="applyCrop" class="bg-white hover:bg-slate-200 text-black px-12 py-4 rounded-none text-xs font-black uppercase tracking-widest shadow-xl transition-all">Apply</button>
                </div>
            </div>
        </div>

        <div v-if="showBetaNotice" class="fixed inset-0 z-[300] bg-black/95 flex items-center justify-center p-6">
            <div class="bg-slate-900 p-12 border border-slate-800 w-full max-w-xl shadow-2xl relative text-center rounded-none">
                <h2 class="text-4xl font-black text-amber-500 mb-6 uppercase tracking-tighter tracking-[0.2em]">BETA</h2>
                <div class="bg-slate-950 p-8 border border-slate-800 mb-10 text-left rounded-none">
                    <strong class="text-red-500 font-black uppercase text-[10px] tracking-widest block mb-3">⚠️ Critical Signal:</strong>
                    <p class="text-xs text-slate-500 leading-relaxed uppercase font-bold tracking-tight">System failure or reset is possible. Maintain local backup. BETA versions may reset database structures without warning.</p>
                </div>
                <div class="flex flex-col items-center gap-6">
                    <button @click="dismissBetaNotice" class="bg-white hover:bg-slate-200 text-black px-12 py-4 rounded-none text-xs font-black uppercase tracking-[0.2em] shadow-xl">Confirm Signal</button>
                    <label class="flex items-center gap-3 text-[10px] font-black uppercase text-slate-600 cursor-pointer transition-colors hover:text-slate-400">
                        <input type="checkbox" v-model="dismissBetaPermanently" class="accent-blue-600 w-5 h-5 rounded-none"> Mute this warning
                    </label>
                </div>
            </div>
        </div>
