...
if ($userCount == 0) {
    ...
    <form method="POST" class="space-y-4">
        <input type="text" name="setup_username" placeholder="Username" required class="w-full bg-slate-950 border border-slate-700 rounded p-2 text-white outline-none">
        <input type="email" name="setup_email" placeholder="Email" required class="w-full bg-slate-950 border border-slate-700 rounded p-2 text-white outline-none">
        <input type="text" name="setup_nickname" placeholder="Display Name" required class="w-full bg-slate-950 border border-slate-700 rounded p-2 text-white outline-none">
        <input type="password" name="setup_password" placeholder="Password" required class="w-full bg-slate-950 border border-slate-700 rounded p-2 text-white outline-none">
        <button type="submit" class="w-full bg-blue-600 p-2 rounded font-bold hover:bg-blue-500 transition">Complete Setup</button>
    </form></div></body></html>
    <?php exit;
}
...
if (isset($_GET['action']) && $_GET['action'] === 'forgot' && empty($_SESSION['logged_in'])) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($u = $stmt->fetch()) {
            ...
            @mail($_POST['forgot_email'], "Password Reset | LunarDesk", "Hello, reset your password here: ".$link);
        }
        $msg = "A reset link has been sent if the email exists.";
    }
    ?>
    <!DOCTYPE html><html><head><title>Forgot Password</title><script src="https://cdn.tailwindcss.com"></script></head>
    ...
    <form method="POST"><input type="email" name="forgot_email" required class="w-full bg-slate-950 border border-slate-700 rounded p-2 mb-4 text-white outline-none">
...
if (empty($_SESSION['logged_in'])) {
    ...
    ?>
    <!DOCTYPE html><html><head><title>Login | LunarDesk</title><script src="https://cdn.tailwindcss.com"></script></head>
    <body class="bg-slate-950 h-screen flex items-center justify-center text-white font-sans"><div class="bg-slate-900 p-8 rounded-xl border border-slate-800 w-96 shadow-2xl">
    <h1 class="text-xl font-bold mb-6 text-center text-blue-500 font-mono tracking-tighter">LUNARDESK ACCESS</h1>
    <?php if(isset($_GET['reset_success'])) echo "<p class='text-green-400 text-sm mb-4 text-center'>Password updated. Please log in.</p>"; ?>
    <?php if(isset($error)) echo "<p class='text-red-400 text-sm mb-4 text-center'>$error</p>"; ?>
    <form method="POST" class="space-y-4">
        <div><label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Username</label>
        <input type="text" name="login_username" required class="w-full bg-slate-950 border border-slate-700 rounded p-2 text-white outline-none focus:border-blue-500 transition"></div>
        <div><div class="flex justify-between items-center mb-1"><label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest">Password</label>
        <a href="?portal=open&action=forgot" class="text-[9px] text-blue-500 font-bold uppercase hover:text-blue-400">Forgot?</a></div>
        <input type="password" name="login_password" required class="w-full bg-slate-950 border border-slate-700 rounded p-2 text-white outline-none focus:border-blue-500 transition"></div>
        <button type="submit" class="w-full bg-blue-600 p-2 rounded font-bold hover:bg-blue-500 transition mt-2">Login</button>
    </form></div></body></html>
    <?php exit;
}
?>